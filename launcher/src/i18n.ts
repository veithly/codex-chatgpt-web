...
const en = {
  ...
  settingTemporaryChat: "Temporary Chat",
  settingTemporaryChatBody: "Run harness turns on the isolated Temporary Chat surface. Turn this off to use normal (history-visible) ChatGPT chats.",
  idleClose: "Idle page close",
  idleCloseBody: "Keep a finished chat page open waiting for a same-session follow-up, then close it after this many idle minutes.",
  minutesUnit: "min",
  gatewaySection: "Gateway",
  gatewayBody: "The local daemon serves any harness on loopback — no API key or OpenAI/Anthropic account needed on the client. ChatGPT login stays in this launcher.",
  gatewayOpenAI: "OpenAI-compatible base URL",
  gatewayAnthropic: "Claude Code: ANTHROPIC_BASE_URL",
  gatewayMcp: "Install as MCP server (Pi / Oh My Pi / ZCode / Claude Code)",
  copyAction: "Copy",
  copied: "Copied",
...
const zh: Record<keyof typeof en, string> = {
  ...
  gatewayOnly: "仅网关模式",
  settingTemporaryChat: "临时会话",
  settingTemporaryChatBody: "在隔离的临时会话页面中运行任务。关闭后改用正常（会出现在历史里）的 ChatGPT 会话。",
  idleClose: "空闲页面关闭",
  idleCloseBody: "对话完成后页面继续保留，等待同一会话的新对话；静默这么久后关闭。",
  minutesUnit: "分钟",
  gatewaySection: "网关",
  gatewayBody: "本地守护进程在回环地址上向任何 harness 提供模型：客户端无需 API Key，也无需 OpenAI/Anthropic 账号；ChatGPT 登录只保留在本启动器内。",
  gatewayOpenAI: "OpenAI 兼容 base URL",
  gatewayAnthropic: "Claude Code：ANTHROPIC_BASE_URL",
  gatewayMcp: "安装为 MCP 服务器（Pi / Oh My Pi / ZCode / Claude Code）",
  copyAction: "复制",
  copied: "已复制",
...
const ja: Record<keyof typeof en, string> = {
  ...
  settingTemporaryChat: "テンポラリーチャット",
  settingTemporaryChatBody: "ターンを隔離されたテンポラリーチャットで実行します。オフにすると通常（履歴に残る）チャットを使用します。",
  idleClose: "アイドルページの閉鎖",
  idleCloseBody: "会話完了後もページを開いたまま同じセッションの追送を待ち、この時間アイドルが続いたら閉じます。",
  minutesUnit: "分",
  gatewaySection: "ゲートウェイ",
  gatewayBody: "ローカルデーモンがループバック上で任意の harness にモデルを提供します。クライアント側に API キーや OpenAI/Anthropic アカウントは不要で、ChatGPT ログインはこのランチャー内だけです。",
  gatewayOpenAI: "OpenAI 互換ベース URL",
  gatewayAnthropic: "Claude Code: ANTHROPIC_BASE_URL",
  gatewayMcp: "MCP サーバーとしてインストール（Pi / Oh My Pi / ZCode / Claude Code）",
  copyAction: "コピー",
  copied: "コピーしました",
...
const ko: Record<keyof typeof en, string> = {
  ...
  settingTemporaryChat: "임시 채팅",
  settingTemporaryChatBody: "터너을 격리된 임시 채팅에서 실행합니다. 끄면 일반(기록에 남는) ChatGPT 채팅을 사용합니다.",
  idleClose: "유휴 페이지 닫기",
  idleCloseBody: "대화가 끝나면 같은 세션의 후속 요청을 기다리고, 이 시간만큼 유휴 상태가 지속되면 닫습니다.",
  minutesUnit: "분",
  gatewaySection: "게이트웨이",
  gatewayBody: "로컬 데몬이 루프백에서 모든 하네스에 모델을 제공합니다. 클라이언트에 API 키나 OpenAI/Anthropic 계정이 필요 없으며 ChatGPT 로그인은 이 런처에만 유지됩니다.",
  gatewayOpenAI: "OpenAI 호환 베이스 URL",
  gatewayAnthropic: "Claude Code: ANTHROPIC_BASE_URL",
  gatewayMcp: "MCP 서버로 설치(Pi / Oh My Pi / ZCode / Claude Code)",
  copyAction: "복사",
  copied: "복사됨",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  gatewayOnly: "僅閘道模式",
  settingTemporaryChat: "暫時會話",
  settingTemporaryChatBody: "在隔離的暫時會話頁面中執行任務。關閉後改用一般（會出現在歷史裡）的 ChatGPT 會話。",
  idleClose: "閒置頁面關閉",
  idleCloseBody: "對話完成後頁面繼續保留，等待同一會話的新對話；靜默這麼久之後關閉。",
  minutesUnit: "分鐘",
  gatewaySection: "閘道",
  gatewayBody: "本機守護行程在回環位址上向任何 harness 提供模型：用戶端無需 API Key，也無需 OpenAI/Anthropic 帳號；ChatGPT 登入只保留在本啟動器內。",
  gatewayOpenAI: "OpenAI 相容 base URL",
  gatewayAnthropic: "Claude Code：ANTHROPIC_BASE_URL",
  gatewayMcp: "安裝為 MCP 伺服器（Pi / Oh My Pi / ZCode / Claude Code）",
  copyAction: "複製",
  copied: "已複製",
